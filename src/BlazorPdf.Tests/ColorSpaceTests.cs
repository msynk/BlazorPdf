...
public class ColorSpaceTests
{
    ...
    public void DeviceGray_midpoint()
        => Assert.Equal((128, 128, 128), BlazorPdfColorSpace.Gray.GetRgb([0.5]));

    [Fact]
    public void DeviceRgb_primary()
        => Assert.Equal(((byte)255, (byte)0, (byte)0), BlazorPdfColorSpace.Rgb.GetRgb([1, 0, 0]));
    ...
    public void DeviceCmyk_to_red()
        ...
        => Assert.Equal(((byte)255, (byte)46, (byte)23), BlazorPdfColorSpace.Cmyk.GetRgb([0, 1, 1, 0]));

    [Fact]
    public void Device_default_components_are_zero()
    {
        Assert.Equal(new double[] { 0, 0, 0 }, BlazorPdfColorSpace.Rgb.DefaultComponents());
        Assert.Single(BlazorPdfColorSpace.Gray.DefaultComponents());
    }

    [Fact]
    public void Lab_black_is_black()
    {
        var lab = BlazorPdfColorSpace.Create(MakeLabArray(), Xref, null);
    ...
    public void Indexed_palette_lookup()
    {
        ...
        var palette = new BlazorPdfString([255, 0, 0, 0, 255, 0]);
        var arr = new List<object?> { BlazorPdfName.Get("Indexed"), BlazorPdfName.Get("DeviceRGB"), 1.0, palette };
        var cs = BlazorPdfColorSpace.Create(arr, Xref, null);
    ...
    private static List<object?> MakeLabArray()
    {
        ...
        return new List<object?> { BlazorPdfName.Get("Lab"), dict };
    }
}
