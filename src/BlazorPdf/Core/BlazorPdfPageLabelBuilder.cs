...
/// </summary>
internal static class BlazorPdfPageLabelBuilder
{
    public static IReadOnlyList<string> Build(IBlazorPdfXRef xref, BlazorPdfDict catalog, int pageCount)
    {
        ...
        if (xref.FetchIfRef(catalog.Get("PageLabels")) is not BlazorPdfDict tree)
        {
            return labels;
        }

        // Collect (startPageIndex -> labelDict) pairs from the number tree.
        var ranges = new SortedDictionary<int, BlazorPdfDict>();
        ...
        for (int r = 0; r < keys.Count; r++)
        {
            ...
            BlazorPdfDict spec = ranges[keys[r]];

            string style = (spec.Get("S") as BlazorPdfName)?.Value ?? "";
            string prefix = (spec.Get("P") as BlazorPdfString)?.AsText() ?? "";
    ...
    private static void CollectNums(IBlazorPdfXRef xref, BlazorPdfDict node, SortedDictionary<int, BlazorPdfDict> ranges, int depth)
    {
        if (depth > 32)
        {
            return;
        ...
        if (xref.FetchIfRef(node.Get("Nums")) is List<object?> nums)
        {
            for (int i = 0; i + 1 < nums.Count; i += 2)
            {
                if (xref.FetchIfRef(nums[i]) is double key
                    && xref.FetchIfRef(nums[i + 1]) is BlazorPdfDict spec)
                {
                    ranges[(int)key] = spec;
                }
            }
        ...
        if (xref.FetchIfRef(node.Get("Kids")) is List<object?> kids)
        {
            ...
            {
                if (xref.FetchIfRef(kidObj) is BlazorPdfDict kid)
                {
