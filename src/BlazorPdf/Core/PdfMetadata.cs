...
public sealed class PdfMetadata
{
    ...
    internal static PdfMetadata Build(IXRef xref, Dict? info, Dict catalog)
    {
        info ??= Dict.Empty;
    ...
    private static string? ReadXmp(IXRef xref, Dict catalog)
    {
        if (xref.FetchIfRef(catalog.Get("Metadata")) is PdfStream stream)
        {
            try
            {
                byte[] bytes = Filters.StreamDecoder.Decode(stream);
