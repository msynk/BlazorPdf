...
/// </summary>
public static class BlazorPdfStreamDecoder
{
    /// <summary>
    ...
    /// </summary>
    public static byte[] Decode(BlazorPdfStream stream, bool inline = false)
    {
        BlazorPdfDict dict = stream.Dict ?? throw new BlazorPdfFormatException("Stream has no dictionary.");
    ...
    private static byte[] ApplyFilter(string name, byte[] data, BlazorPdfDict? parms)
    {
        switch (name)
        {
            case "FlateDecode":
            ...
            case "LZWDecode":
            case "LZW":
                return ApplyPredictorIfAny(BlazorPdfLzwDecode.Decode(data, EarlyChange(parms)), parms);
            case "ASCIIHexDecode":
            case "AHx":
    ...
    private static byte[] ApplyPredictorIfAny(byte[] data, BlazorPdfDict? parms)
    {
        if (parms is null)
        {
            return data;
        ...
        return BlazorPdfPredictor.Apply(data, predictor, colors, bpc, columns);
    }

    private static int EarlyChange(BlazorPdfDict? parms) => ToInt(parms?.Get("EarlyChange"), 1);
    ...
    private static List<string> ResolveNames(object? filter, IBlazorPdfXRef? xref)
    {
        var result = new List<string>();
        switch (filter)
        {
            case BlazorPdfName n:
                AddFilterName(result, n.Value);
                break;
            case List<object?> arr:
                foreach (var item in arr)
                {
                    if ((xref?.FetchIfRef(item) ?? item) is BlazorPdfName name)
                    {
    ...
    private static List<BlazorPdfDict?> ResolveParms(object? parms, int count, IBlazorPdfXRef? xref)
    {
        var result = new List<BlazorPdfDict?>(count);
        if (parms is List<object?> arr)
        {
            for (int i = 0; i < count; i++)
            {
                result.Add(i < arr.Count ? xref?.FetchIfRef(arr[i]) as BlazorPdfDict ?? arr[i] as BlazorPdfDict : null);
            }
        }
        else
        {
            result.Add(parms as BlazorPdfDict);
