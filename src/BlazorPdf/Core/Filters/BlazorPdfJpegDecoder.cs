...
internal static class BlazorPdfJpegDecoder
{
    ...
    public static BlazorPdfJpegImage? Decode(byte[] data)
    {
        try
        {
    ...
    private static BlazorPdfJpegImage? DecodeCore(byte[] data)
    {
        int pos = 0;
        var quant = new int[4][];
    ...
    private static BlazorPdfJpegImage AssembleImage(int width, int height, Component[] components,
        bool adobe, int adobeTransform)
    {
        ...
        return new BlazorPdfJpegImage { Width = width, Height = height, Components = nc, Data = data };
    }
