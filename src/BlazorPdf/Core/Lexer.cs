...
/// strings, booleans, <c>null</c>, command keywords (<see cref="Cmd"/>) and the
...
public sealed class Lexer
{
    ...
    public Lexer(BaseStream stream)
    {
        _stream = stream;
    ...
    public BaseStream Stream => _stream;
    ...
    /// <summary>
    /// Reads and returns the next object token. Returns <see cref="Primitives.EOF"/>
    /// at end of stream.
    /// </summary>
    public object GetObj()
    {
        ...
        while ((ch = NextChar()) >= 0 && IsRegular(ch))
        {
            ...
            if (sb.Length == 128)
            {
                throw new PdfFormatException("Command token too long.");
            }
        }
    ...
    private PdfString GetHexString()
    {
        var bytes = new List<byte>();
        int ch = _currentChar;
        int hi = -1;
