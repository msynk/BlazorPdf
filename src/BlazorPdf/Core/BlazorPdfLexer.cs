...
/// strings, booleans, <c>null</c>, command keywords (<see cref="BlazorPdfCmd"/>) and the
/// structural delimiters <c>[ ] &lt;&lt; &gt;&gt; { }</c> (emitted as commands).
/// </summary>
public sealed class BlazorPdfLexer
{
    private readonly BlazorPdfBaseStream _stream;
    ...
    public BlazorPdfLexer(BlazorPdfBaseStream stream)
    {
        _stream = stream;
    ...
    public BlazorPdfBaseStream Stream => _stream;
    ...
    /// <summary>
    /// Reads and returns the next object token. Returns <see cref="BlazorPdfPrimitives.EOF"/>
    /// at end of stream.
    /// </summary>
    public object GetObj()
    {
        ...
        while (true)
        {
            if (ch < 0)
            {
                return BlazorPdfPrimitives.EOF;
            }
            if (comment)
            {
        ...
        while ((ch = NextChar()) >= 0 && IsRegular(ch))
        {
            ...
            if (sb.Length == 128)
            {
                throw new BlazorPdfFormatException("Command token too long.");
            }
        }
        ...
        return keyword switch
        {
            "true" => true,
            "false" => false,
            "null" => BlazorPdfPrimitives.Null,
            _ => BlazorPdfCmd.Get(keyword),
        };
    }
    ...
    private BlazorPdfString GetString()
    {
        ...
        return new BlazorPdfString(bytes.ToArray());
    }

    private BlazorPdfName GetName()
    {
        var sb = new StringBuilder();
        int ch;
        while ((ch = NextChar()) >= 0 && IsRegular(ch))
        ...
        return BlazorPdfName.Get(sb.ToString());
    }

    private BlazorPdfString GetHexString()
    {
        var bytes = new List<byte>();
        int ch = _currentChar;
        int hi = -1;
