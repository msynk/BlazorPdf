...
/// <summary>
/// Renders a <see cref="BlazorPdfPage"/> to an HTML fragment. The page is a single
...
public sealed class BlazorPdfHtmlRenderer
{
    ...
    private readonly Dictionary<object, BlazorPdfFont> _fontCache;
    ...
    private BlazorPdfGraphicsState _state = new();
    private readonly Stack<BlazorPdfGraphicsState> _stack = new();
    private readonly Stack<int> _groupDepthStack = new();
    private BlazorPdfDict? _resources;
    private BlazorPdfMatrix _baseMatrix = BlazorPdfMatrix.Identity;
    ...
    private BlazorPdfMatrix _textMatrix = BlazorPdfMatrix.Identity;
    private BlazorPdfMatrix _textLineMatrix = BlazorPdfMatrix.Identity;
    ...
    /// <summary>
    /// How painted text runs are emitted. <see cref="BlazorPdfTextCoalescing.Compact"/>
    ...
    /// <see cref="BlazorPdfTextCoalescing.Exact"/>.
    /// </summary>
    public BlazorPdfTextCoalescing TextCoalescing { get; set; } = BlazorPdfTextCoalescing.Exact;
    ...
    /// overlays. Used through <see cref="BlazorPdfCanvasRenderer"/>.
    /// </summary>
    ...
    public BlazorPdfHtmlRenderer(BlazorPdfPage page, IBlazorPdfXRef xref, int rotationOffset = 0)
    ...
    /// (retrieve it via <see cref="BlazorPdfFontStore.FontFaceStyle"/>). When
    /// <paramref name="fontStore"/> is <c>null</c> the page is self-contained.
    /// </summary>
    public BlazorPdfHtmlRenderer(BlazorPdfPage page, IBlazorPdfXRef xref, BlazorPdfFontStore? fontStore, int rotationOffset = 0)
    {
        _page = page;
        _xref = xref;
        ...
        {
            _fontCache = new Dictionary<object, BlazorPdfFont>();
    ...
    public string Render()
    {
        ...
        (BlazorPdfMatrix baseMatrix, double viewW, double viewH) = BuildViewport(
        ...
        try
        {
            ops = new BlazorPdfContentParser(_page.GetContentBytes()).Parse();
        }
        catch (Exception ex)
        {
            ops = new List<BlazorPdfOperation>();
    ...
    private void RunOps(List<BlazorPdfOperation> ops)
    {
    ...
    private static (BlazorPdfMatrix, double, double) BuildViewport(
        int rotate, double x0, double y0, double x1, double y1, double w, double h)
    {
        return rotate switch
        {
            90 => (new BlazorPdfMatrix(0, 1, 1, 0, -y0, -x0), h, w),
            180 => (new BlazorPdfMatrix(-1, 0, 0, 1, x1, -y0), w, h),
            270 => (new BlazorPdfMatrix(0, -1, -1, 0, y1, x1), h, w),
            _ => (new BlazorPdfMatrix(1, 0, 0, -1, -x0, y1), w, h),
        };
    }
    ...
    private void Execute(BlazorPdfOperation op)
    {
        ...
        switch (op.Operator)
        {
            ...
            case "cm":
                ...
                {
                    _state.Ctm = BlazorPdfMatrix.Concat(_state.Ctm,
                        new BlazorPdfMatrix(op.Num(0), op.Num(1), op.Num(2), op.Num(3), op.Num(4), op.Num(5)));
                }
                break;
            ...
            case "g": _state.FillColorSpace = BlazorPdfColorSpace.Gray; _state.FillColor = Gray(op.Num(0)); _state.FillPattern = null; break;
            case "G": _state.StrokeColorSpace = BlazorPdfColorSpace.Gray; _state.StrokeColor = Gray(op.Num(0)); break;
            case "rg": _state.FillColorSpace = BlazorPdfColorSpace.Rgb; _state.FillColor = Rgb(op.Num(0), op.Num(1), op.Num(2)); _state.FillPattern = null; break;
            case "RG": _state.StrokeColorSpace = BlazorPdfColorSpace.Rgb; _state.StrokeColor = Rgb(op.Num(0), op.Num(1), op.Num(2)); break;
            case "k": _state.FillColorSpace = BlazorPdfColorSpace.Cmyk; _state.FillColor = Cmyk(op.Num(0), op.Num(1), op.Num(2), op.Num(3)); _state.FillPattern = null; break;
            case "K": _state.StrokeColorSpace = BlazorPdfColorSpace.Cmyk; _state.StrokeColor = Cmyk(op.Num(0), op.Num(1), op.Num(2), op.Num(3)); break;
            case "sc":
            ...
            // Text objects.
            case "BT":
                _textMatrix = BlazorPdfMatrix.Identity;
                _textLineMatrix = BlazorPdfMatrix.Identity;
                break;
            case "ET": break;
            ...
            case "Tm":
                _textLineMatrix = new BlazorPdfMatrix(op.Num(0), op.Num(1), op.Num(2), op.Num(3), op.Num(4), op.Num(5));
    ...
    private void PaintShading(BlazorPdfOperation op)
    {
        if (op.Operands.Count == 0 || op.Operands[0] is not BlazorPdfName name)
        {
            return;
        }
        if (_resources?.Get("Shading") is not BlazorPdfDict shadings)
        {
            return;
        }
        object? shadingObj = shadings.Get(name.Value);
        BlazorPdfDict? shading = shadingObj as BlazorPdfDict ?? (shadingObj as BlazorPdfStream)?.Dict;
        if (shading is null)
        {
            return;
        }
        ...
        if (_ops is not null)
        {
            ...
            object?[]? grad = BlazorPdfCssShadingBuilder.BuildCanvasOp(shading, _xref, _resources, _state.Ctm);
            if (grad is not null)
            {
        ...
        string? background = BlazorPdfCssShadingBuilder.Build(shading, _xref, _resources, _state.Ctm, _viewW, _viewH);
    ...
    private void DoXObject(BlazorPdfOperation op)
    {
        if (op.Operands.Count == 0 || op.Operands[0] is not BlazorPdfName name)
        {
            return;
        }
        if (_resources?.Get("XObject") is not BlazorPdfDict xobjects || xobjects.Get(name.Value) is not BlazorPdfStream stream
            || stream.Dict is null)
        {
            return;
        }

        string subtype = (stream.Dict.Get("Subtype") as BlazorPdfName)?.Value ?? "";
        if (subtype == "Image")
        {
            DrawImage(stream);
        }
    ...
    private void DrawImage(BlazorPdfStream stream)
    {
        var fill = ParseRgb(_state.FillColor);
        string? uri = BlazorPdfImage.BuildDataUri(stream, _xref, _resources, fill);
        if (uri is null)
        {
            return;
        }
    ...
    private void DrawInlineImage(BlazorPdfOperation op)
    {
        if (op.Operands.Count < 2 || op.Operands[0] is not BlazorPdfDict dict || op.Operands[1] is not byte[] data)
        {
            return;
        }
        var stream = new BlazorPdfStream(data, 0, data.Length, dict);
        var fill = ParseRgb(_state.FillColor);
        string? uri = BlazorPdfImage.BuildDataUri(stream, _xref, _resources, fill, inline: true);
        if (uri is not null)
        {
    ...
    private static bool Interpolate(BlazorPdfDict dict) => dict.Get("Interpolate", "I") is bool b && b;
    ...
    private void EmitImage(string uri, int pixelW, int pixelH, bool interpolate)
    {
        ...
        BlazorPdfMatrix unit = BlazorPdfMatrix.Concat(_state.Ctm, new BlazorPdfMatrix(1, 0, 0, -1, 0, 1));
        BlazorPdfMatrix m = BlazorPdfMatrix.Concat(unit, new BlazorPdfMatrix(1.0 / pixelW, 0, 0, 1.0 / pixelH, 0, 0));
    ...
    private static int PixelSize(BlazorPdfDict dict, string key1, string key2)
        => dict.Get(key1, key2) is double d ? (int)d : 0;

    // ----- Optional content (OCG) -----

    private void BeginMarkedContent(BlazorPdfOperation op)
    {
        _mcDepth++;
        ...
        if (_ocHiddenAtDepth < 0 && op.Operands.Count >= 2
            && op.Operands[0] is BlazorPdfName tag && tag.Value == "OC"
    ...
    {
        object? raw = operand;
        if (operand is BlazorPdfName propName && _resources?.Get("Properties") is BlazorPdfDict props)
        {
            raw = props.GetRaw(propName.Value);
        }
        if (_xref.FetchIfRef(raw) is not BlazorPdfDict ocg)
        {
            return false;
        }

        // An OCMD references one or more OCGs; hidden only when every member is off.
        if (BlazorPdfPrimitives.IsName(ocg.Get("Type"), "OCMD"))
        {
    ...
    private bool IsOcgOff(object? raw)
    {
        if (_ocgOff is null)
        {
            ...
            if ((_xref as BlazorPdfXRef)?.Root?.Get("OCProperties") is BlazorPdfDict ocp && ocp.Get("D") is BlazorPdfDict cfg
                && cfg.Get("OFF") is List<object?> off)
            {
                foreach (var item in off)
                {
                    if (item is BlazorPdfRef r)
                    {
        ...
        return raw is BlazorPdfRef rf && _ocgOff.Contains(rf.ToRefString());
    }

    private void DrawForm(BlazorPdfStream stream)
    {
        ...
        _stack.Push(_state.Clone());
        _groupDepthStack.Push(_openGroups);
        BlazorPdfDict? savedResources = _resources;

        if (stream.Dict!.Get("Matrix") is List<object?> mtx && mtx.Count >= 6)
        {
            _state.Ctm = BlazorPdfMatrix.Concat(_state.Ctm, new BlazorPdfMatrix(
                Num(mtx[0]), Num(mtx[1]), Num(mtx[2]), Num(mtx[3]), Num(mtx[4]), Num(mtx[5])));
        }

        if (stream.Dict.Get("Resources") is BlazorPdfDict formResources)
        {
        ...
        if (_ops is null
            && stream.Dict.Get("Group") is BlazorPdfDict grp && BlazorPdfPrimitives.IsName(grp.Get("S"), "Transparency")
        ...
        try
        {
            byte[] content = BlazorPdfStreamDecoder.Decode(stream);
            RunOps(new BlazorPdfContentParser(content).Parse());
        }
        catch
        {
    ...
    private void RenderAnnotations()
    {
        ...
        {
            if (_xref.FetchIfRef(item) is not BlazorPdfDict annot)
            {
                continue;
            }
    ...
    private void DrawAnnotationAppearance(BlazorPdfDict annot)
    {
        if (annot.Get("Rect") is not List<object?> rectArr || rectArr.Count < 4)
        {
            return;
        }
        BlazorPdfStream? appearance = ResolveAppearance(annot);
        ...
        BlazorPdfMatrix formMatrix = appearance.Dict.Get("Matrix") is List<object?> m && m.Count >= 6
            ? new BlazorPdfMatrix(Num(m[0]), Num(m[1]), Num(m[2]), Num(m[3]), Num(m[4]), Num(m[5]))
            : BlazorPdfMatrix.Identity;

        BlazorPdfMatrix a = ComputeAppearanceMatrix(bbox, formMatrix, rect);

        // Reset to a clean state anchored at the page base transform.
        _stack.Clear();
        _groupDepthStack.Clear();
        _state = new BlazorPdfGraphicsState { Ctm = BlazorPdfMatrix.Concat(_baseMatrix, a) };
    ...
    private BlazorPdfStream? ResolveAppearance(BlazorPdfDict annot)
    {
        if (annot.Get("AP") is not BlazorPdfDict ap)
        {
            return null;
        }
        object? normal = ap.Get("N");
        if (normal is BlazorPdfStream stream)
        {
            return stream;
        }
        // Appearance sub-dictionary keyed by the current appearance state (/AS).
        if (normal is BlazorPdfDict states)
        {
            string? state = (annot.Get("AS") as BlazorPdfName)?.Value;
            if (state is not null && states.Get(state) is BlazorPdfStream selected)
            {
                return selected;
            }
            foreach (var key in states.Keys)
            {
                if (states.Get(key) is BlazorPdfStream first)
                {
                    return first;
                }
            }
    ...
    private void DrawLinkOverlay(BlazorPdfDict annot)
    {
        if (!BlazorPdfPrimitives.IsName(annot.Get("Subtype"), "Link"))
        {
            return;
        }
        if (annot.Get("Rect") is not List<object?> rectArr || rectArr.Count < 4)
        ...
        if (action is BlazorPdfDict actionDict && BlazorPdfPrimitives.IsName(actionDict.Get("S"), "URI")
            && actionDict.Get("URI") is BlazorPdfString u)
        {
            uri = u.AsLatin1();
        }
        ...
        if (uri is null && DestinationResolver is not null)
        {
            ...
            if (dest is null && action is BlazorPdfDict a && BlazorPdfPrimitives.IsName(a.Get("S"), "GoTo"))
            {
                dest = a.Get("D");
            }
            if (dest is not null)
    ...
    {
        BlazorPdfMatrix transform = BlazorPdfMatrix.Concat(_baseMatrix, new BlazorPdfMatrix(1, 0, 0, 1, r[0], r[1]));
    ...
    /// </summary>
    private List<double[]>? QuadPointRegions(BlazorPdfDict annot)
    {
    ...
    private static BlazorPdfMatrix ComputeAppearanceMatrix(double[] bbox, BlazorPdfMatrix formMatrix, double[] rect)
    {
        ...
        return new BlazorPdfMatrix(sx, 0, 0, sy, rect[0] - minX * sx, rect[1] - minY * sy);
    }
    ...
    private void SetFont(BlazorPdfOperation op)
    {
        if (op.Operands.Count < 2 || op.Operands[0] is not BlazorPdfName fontName)
        {
            return;
        }
    ...
    private BlazorPdfFont? ResolveFont(string name)
    {
        if (_resources?.Get("Font") is not BlazorPdfDict fonts)
        {
            return null;
        }
        ...
        object cacheKey = raw is BlazorPdfRef r ? r : _xref.FetchIfRef(raw) ?? name;
        if (_fontCache.TryGetValue(cacheKey, out var cached))
        {
            return cached;
        }
        if (_xref.FetchIfRef(raw) is BlazorPdfDict fontDict)
        {
            var font = BlazorPdfFont.Create(fontDict, _xref);
    ...
    {
        _textLineMatrix = BlazorPdfMatrix.Concat(_textLineMatrix, new BlazorPdfMatrix(1, 0, 0, 1, tx, ty));
        _textMatrix = _textLineMatrix;
    }
    ...
    private void ShowTextArray(List<object?>? array)
    {
        ...
        foreach (var item in array)
        {
            ...
            else if (item is double adjust)
            {
                ...
                _textMatrix = BlazorPdfMatrix.Concat(_textMatrix, new BlazorPdfMatrix(1, 0, 0, 1, tx, 0));
            }
        }
    }
    ...
    {
        if (operand is not BlazorPdfString s || _state.Font is null || _state.FontSize == 0)
        {
            return;
        }
        ...
        _textMatrix = BlazorPdfMatrix.Concat(_textMatrix, new BlazorPdfMatrix(1, 0, 0, 1, displacement, 0));
    }
    ...
    /// </summary>
    private void ShowType3Text(List<BlazorPdfGlyph> glyphs)
    {
        ...
        BlazorPdfFont font = _state.Font!;
        BlazorPdfMatrix fontMatrix = font.Type3!.FontMatrix;
    ...
    private void RenderType3Glyph(BlazorPdfStream proc, BlazorPdfMatrix fontMatrix, BlazorPdfDict? glyphResources)
    {
        if (_formDepth >= MaxFormDepth)
        {
            return;
        ...
        BlazorPdfMatrix trm = BlazorPdfMatrix.Concat(BlazorPdfMatrix.Concat(_state.Ctm, _textMatrix),
            new BlazorPdfMatrix(_state.FontSize * _state.HorizScale, 0, 0, _state.FontSize, 0, _state.TextRise));
        BlazorPdfMatrix glyphToDevice = BlazorPdfMatrix.Concat(trm, fontMatrix);
        ...
        try
        {
            byte[] content = BlazorPdfStreamDecoder.Decode(proc);
            RunOps(new BlazorPdfContentParser(content).Parse());
        }
        catch
        {
    ...
    private void EmitText(string renderText, string realText, double runAdvance)
    {
        ...
        if (TextCoalescing == BlazorPdfTextCoalescing.Compact
            && a > 1e-3 && d > 1e-3
    ...
    /// </summary>
    private void EmitCanvasText(string text, BlazorPdfMatrix trm, double fontHeight, double targetWidth,
    ...
    private void AccumulatePaintedRun(string text, bool glyphMapped,
        BlazorPdfMatrix trm, double left, double top,
    ...
    /// </summary>
    private void AccumulateSelectionText(string text, BlazorPdfMatrix trm, double left, double top,
    ...
    private void AppendFontStyle(StringBuilder sb, BlazorPdfFont font)
    {
    ...
    /// </summary>
    private string FontFamilyList(BlazorPdfFont font)
    {
        if (!font.HasEmbedded)
        {
    ...
    private void ApplyExtGState(BlazorPdfOperation op)
    {
        if (op.Operands.Count == 0 || op.Operands[0] is not BlazorPdfName name)
        {
            return;
        }
        if (_resources?.Get("ExtGState") is BlazorPdfDict ext && ext.Get(name.Value) is BlazorPdfDict gs)
        {
            ...
            string? bmName = bm switch
            {
                ...
                List<object?> arr when arr.Count > 0 && arr[0] is BlazorPdfName first => first.Value,
                _ => null,
            };
    ...
    private void SetFillColorN(BlazorPdfOperation op)
    {
        // scn/sc with a trailing name selects a pattern; otherwise it's a color.
        if (op.Operands.Count > 0 && op.Operands[^1] is BlazorPdfName pattern)
        {
    ...
    private void SetStrokeColorN(BlazorPdfOperation op)
    {
        if (op.Operands.Count > 0 && op.Operands[^1] is BlazorPdfName)
        {
    ...
    private BlazorPdfColorSpace? ResolveColorSpace(BlazorPdfOperation op)
    {
        if (op.Operands.Count == 0 || op.Operands[0] is not BlazorPdfName name)
        {
            return null;
        ...
        {
            "DeviceGray" or "G" => BlazorPdfColorSpace.Gray,
            "DeviceRGB" or "RGB" => BlazorPdfColorSpace.Rgb,
            "DeviceCMYK" or "CMYK" => BlazorPdfColorSpace.Cmyk,
            "Pattern" => null,
            _ => BlazorPdfColorSpace.Create(name, _xref, _resources),
        };
    }
    ...
    {
        BlazorPdfColorSpace? cs = stroke ? _state.StrokeColorSpace : _state.FillColorSpace;
    ...
    /// </summary>
    private static string? ColorViaSpace(BlazorPdfColorSpace? cs, BlazorPdfOperation op)
    {
    ...
    private void SetDash(BlazorPdfOperation op)
    {
    ...
    private string? ResolveFillPaint(string name)
    {
        ...
        if (_resources?.Get("Pattern") is BlazorPdfDict patterns)
        {
            object? raw = patterns.GetRaw(name);
            cacheKey = raw is BlazorPdfRef r ? r : _xref.FetchIfRef(raw) ?? name;
        }
    ...
    private string? BuildPattern(string name)    {
        if (_resources?.Get("Pattern") is not BlazorPdfDict patterns)
        {
            return null;
        }
        object? obj = patterns.Get(name);
        BlazorPdfDict? patDict = obj as BlazorPdfDict ?? (obj as BlazorPdfStream)?.Dict;
        if (patDict is null)
        {
            return null;
        }

        BlazorPdfMatrix matrix = patDict.Get("Matrix") is List<object?> m && m.Count >= 6
            ? new BlazorPdfMatrix(Num(m[0]), Num(m[1]), Num(m[2]), Num(m[3]), Num(m[4]), Num(m[5]))
            : BlazorPdfMatrix.Identity;
        ...
        if (patternType == 2)
        {
            ...
            return BlazorPdfCssShadingBuilder.Build(shading, _xref, _resources,
                BlazorPdfMatrix.Concat(_baseMatrix, matrix), _viewW, _viewH);
        }
        return null;
    }
    ...
    {
        if (_resources?.Get("Pattern") is not BlazorPdfDict patterns
            || patterns.Get(_state.FillPattern!) is not BlazorPdfStream patStream
            || patStream.Dict is null)
        {
            return false;
        }

        BlazorPdfDict pd = patStream.Dict;
        ...
        BlazorPdfMatrix patternMatrix = pd.Get("Matrix") is List<object?> m && m.Count >= 6
            ? new BlazorPdfMatrix(Num(m[0]), Num(m[1]), Num(m[2]), Num(m[3]), Num(m[4]), Num(m[5]))
            : BlazorPdfMatrix.Identity;
        BlazorPdfMatrix patToDevice = BlazorPdfMatrix.Concat(_baseMatrix, patternMatrix);
        if (patToDevice.Invert() is not BlazorPdfMatrix inv)
        {
            return false;
        }
        ...
        double[] bbox = pd.Get("BBox") is List<object?> bb && bb.Count >= 4
            ...
            : [0, 0, xStep, yStep];
        BlazorPdfDict? patRes = pd.Get("Resources") as BlazorPdfDict;
        ...
        for (int j = jStart; j <= jEnd; j++)
        {
            ...
            {
                BlazorPdfMatrix cellCtm = BlazorPdfMatrix.Concat(patToDevice, new BlazorPdfMatrix(1, 0, 0, 1, i * xStep, j * yStep));
    ...
    private void RunPatternCell(BlazorPdfStream patStream, BlazorPdfMatrix cellCtm, BlazorPdfDict? patRes, double[] bbox)
    {
        if (_formDepth >= MaxFormDepth)
        {
            return;
        ...
        try
        {
            byte[] content = BlazorPdfStreamDecoder.Decode(patStream);
            RunOps(new BlazorPdfContentParser(content).Parse());
        }
        catch
        {
    ...
    private static string Cmyk(double c, double m, double y, double k)
    {
        ...
        var (r, g, b) = BlazorPdfColorSpace.CmykToRgb(c, m, y, k);
        return $"rgb({r},{g},{b})";
    }

    private static string? ColorFromComponents(BlazorPdfOperation op)
    {
