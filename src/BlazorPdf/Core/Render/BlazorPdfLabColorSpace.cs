...
/// </summary>
internal sealed class BlazorPdfLabColorSpace : BlazorPdfColorSpace
{
    private readonly double _xw, _yw, _zw;
    private readonly double _amin, _amax, _bmin, _bmax;

    private BlazorPdfLabColorSpace(double xw, double yw, double zw, double[] range)
    {
    ...
    public static BlazorPdfLabColorSpace Build(List<object?> arr, IBlazorPdfXRef xref)
    {
        double xw = 1, yw = 1, zw = 1;
        double[] range = [-100, 100, -100, 100];
        if (xref.FetchIfRef(arr.Count > 1 ? arr[1] : null) is BlazorPdfDict dict)
        {
        ...
        return new BlazorPdfLabColorSpace(xw, yw, zw, range);
    }
