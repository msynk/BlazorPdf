...
internal static class PdfImage
{
    public static string? BuildDataUri(PdfStream stream, IXRef xref, Dict? resources, (byte R, byte G, byte B) fillColor)
    {
        ...
        try
        {
            ...
            else
            {
                ...
                if (maskObj is PdfStream stencil)
                {
    ...
    private static void DecodeColorImage(byte[] data, int width, int height, int bpc,
        ColorSpace cs, byte[] rgba, double[]? decode, int[]? colorKey = null)
    {
    ...
    private static void ApplyStencilMask(PdfStream mask, IXRef xref, int width, int height, byte[] rgba)
    {
        ...
        try
        {
            mdata = StreamDecoder.Decode(mask);
            if (filters.Contains("CCITTFaxDecode") || filters.Contains("CCF"))
            {
                mdata = CcittFaxDecoder.Decode(mdata, ReadCcittParams(mask.Dict, xref, mw, mh));
            }
        }
        catch
        {
    ...
    private static double[]? ReadRawDecode(Dict dict)
    {
    ...
    private static byte[]? JpegToRgba(JpegImage jpeg, Dict dict, IXRef xref)
    {
        ...
        for (int i = 0; i < w * h; i++)
        {
            ...
            (byte r, byte g, byte b) = nc switch
            {
                ...
                4 => ColorSpace.CmykToRgb(comps[0], comps[1], comps[2], comps[3]),
    ...
    private static double[]? ReadDecodeArray(object? value, IXRef xref)
    {
