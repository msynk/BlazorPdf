...
/// </summary>
internal static class BlazorPdfCssShadingBuilder
{
    ...
    /// </summary>
    public static string? Build(BlazorPdfDict shading, IBlazorPdfXRef xref, BlazorPdfDict? resources, BlazorPdfMatrix ctm,
        double viewW, double viewH)
    {
        int type = shading.Get("ShadingType") is double d ? (int)d : 0;
        var cs = BlazorPdfColorSpace.Create(shading.Get("ColorSpace"), xref, resources);
        var fn = BlazorPdfFunction.Create(shading.Get("Function"), xref);
    ...
    /// </summary>
    public static object?[]? BuildCanvasOp(BlazorPdfDict shading, IBlazorPdfXRef xref, BlazorPdfDict? resources, BlazorPdfMatrix ctm)
    {
        int type = shading.Get("ShadingType") is double d ? (int)d : 0;
        var cs = BlazorPdfColorSpace.Create(shading.Get("ColorSpace"), xref, resources);
        var fn = BlazorPdfFunction.Create(shading.Get("Function"), xref);
    ...
    /// </summary>
    private static object?[] CanvasStops(double[] domain, BlazorPdfColorSpace cs, BlazorPdfFunction? fn,
    ...
    private static string? BuildFallback(BlazorPdfDict shading, double[] domain, BlazorPdfColorSpace cs, BlazorPdfFunction? fn)
    {
        if (fn is not null)
        {
            long r = 0, g = 0, b = 0;
    ...
    private static string BuildAxial(double[] c, double[] domain, BlazorPdfColorSpace cs, BlazorPdfFunction? fn,
        BlazorPdfMatrix ctm, double viewW, double viewH, bool extendBefore, bool extendAfter)
    {
    ...
    private static string BuildRadial(double[] c, double[] domain, BlazorPdfColorSpace cs, BlazorPdfFunction? fn,
        BlazorPdfMatrix ctm, bool extendBefore, bool extendAfter)
    {
        double scale = ctm.ScaleFactor;
    ...
    private static (int R, int G, int B) SampleRgb(double[] domain, double frac, BlazorPdfColorSpace cs, BlazorPdfFunction? fn)
    {
    ...
    private static double[] ReadNumbers(object? value, IBlazorPdfXRef? xref = null)
    {
        ...
        for (int i = 0; i < arr.Count; i++)
        {
            ...
            result[i] = BlazorPdfPrimitives.ResolveNumber(xref, arr[i]);
        }
        return result;
    }
}
