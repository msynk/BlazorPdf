...
/// </summary>
public abstract class BlazorPdfColorSpace
{
    ...
    public static BlazorPdfColorSpace Create(object? obj, IBlazorPdfXRef xref, BlazorPdfDict? resources)
    {
        obj = xref.FetchIfRef(obj);

        if (obj is BlazorPdfName name)
        {
            switch (name.Value)
            {
                case "DeviceGray":
            ...
            if (resources?.Get("ColorSpace") is BlazorPdfDict csDict && csDict.Has(name.Value))
            {
        ...
        {
            string kind = (xref.FetchIfRef(arr[0]) as BlazorPdfName)?.Value ?? "";
            switch (kind)
            {
                case "ICCBased":
                    if (xref.FetchIfRef(arr[1]) is BlazorPdfStream icc && icc.Dict is not null)
                    {
                ...
                case "CalRGB":
                    return Rgb;
                case "Lab":
                    return BlazorPdfLabColorSpace.Build(arr, xref);
                case "CalGray":
                    return Gray;
                case "Indexed":
                case "I":
                    return BlazorPdfIndexedColorSpace.Build(arr, xref, resources);
                case "Separation":
                case "DeviceN":
                    return BlazorPdfSeparationColorSpace.Build(arr, xref, resources);
                case "Pattern":
    ...
    public static readonly BlazorPdfColorSpace Gray = new DeviceGray();
    public static readonly BlazorPdfColorSpace Rgb = new DeviceRgb();
    public static readonly BlazorPdfColorSpace Cmyk = new DeviceCmyk();
    ...
    private static BlazorPdfColorSpace? ResolveDefault(BlazorPdfDict? resources, string key, IBlazorPdfXRef xref)
    {
        if (resources?.Get("ColorSpace") is not BlazorPdfDict csDict || !csDict.Has(key))
        {
            return null;
        }
        if (xref.FetchIfRef(csDict.Get(key)) is BlazorPdfName dn &&
    ...
    private sealed class DeviceGray : BlazorPdfColorSpace
    {
    ...
    private sealed class DeviceRgb : BlazorPdfColorSpace
    {
    ...
    private sealed class DeviceCmyk : BlazorPdfColorSpace
    {
