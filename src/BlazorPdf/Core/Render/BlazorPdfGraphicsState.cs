...
/// </summary>
public sealed class BlazorPdfGraphicsState
{
    public BlazorPdfMatrix Ctm { get; set; } = BlazorPdfMatrix.Identity;

    public string FillColor { get; set; } = "rgb(0,0,0)";
    public string StrokeColor { get; set; } = "rgb(0,0,0)";
    public BlazorPdfColorSpace? FillColorSpace { get; set; }   // current fill space (cs)
    public BlazorPdfColorSpace? StrokeColorSpace { get; set; } // current stroke space (CS)
    ...
    // Text state.
    public BlazorPdfFont? Font { get; set; }
    ...
    public BlazorPdfGraphicsState Clone() => new()
    {
        Ctm = Ctm,
        FillColor = FillColor,
