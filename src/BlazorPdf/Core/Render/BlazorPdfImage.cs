...
/// </summary>
internal static class BlazorPdfImage
{
    public static string? BuildDataUri(BlazorPdfStream stream, IBlazorPdfXRef xref, BlazorPdfDict? resources, (byte R, byte G, byte B) fillColor, bool inline = false)
    {
        ...
        try
        {
            data = BlazorPdfStreamDecoder.Decode(stream, inline);
        }
        catch
        {
            return null;
        ...
        if (isJpeg)
        {
            ...
            BlazorPdfColorSpace jcs = BlazorPdfColorSpace.Create(dict.Get("ColorSpace", "CS"), xref, resources);
            bool needsCSharp = jcs.Components == 4
                || dict.Get("SMask") is BlazorPdfStream
                || xref.FetchIfRef(dict.Get("Mask")) is not null
                || dict.Get("Decode", "D") is List<object?>;

            if (needsCSharp && BlazorPdfJpegDecoder.Decode(data) is { } jpeg)
            {
                byte[]? jrgba = JpegToRgba(jpeg, dict, xref);
                if (jrgba is not null)
                {
                    ApplySoftMask(dict, xref, resources, jpeg.Width, jpeg.Height, jrgba);
                    if (xref.FetchIfRef(dict.Get("Mask")) is BlazorPdfStream jstencil)
                    {
                        ApplyStencilMask(jstencil, xref, jpeg.Width, jpeg.Height, jrgba);
                    }
                    byte[] jpng = BlazorPdfPngEncoder.EncodeRgba(jpeg.Width, jpeg.Height, jrgba);
        ...
        if (filterNames.Contains("CCITTFaxDecode") || filterNames.Contains("CCF"))
        {
            try
            {
                data = BlazorPdfCcittFaxDecoder.Decode(data, ReadCcittParams(dict, xref, width, height));
            }
            catch
            {
                return null;
        ...
        try
        {
            ...
            {
                BlazorPdfColorSpace cs = BlazorPdfColorSpace.Create(dict.Get("ColorSpace", "CS"), xref, resources);
                ...
                if (maskObj is BlazorPdfStream stencil)
                {
                    ApplyStencilMask(stencil, xref, width, height, rgba);
                }
            }

            byte[] png = BlazorPdfPngEncoder.EncodeRgba(width, height, rgba);
    ...
    private static void DecodeImageMask(byte[] data, int width, int height, BlazorPdfDict dict,
    ...
    private static void DecodeColorImage(byte[] data, int width, int height, int bpc,
        BlazorPdfColorSpace cs, byte[] rgba, double[]? decode, int[]? colorKey = null)
    {
        int nComps = cs.Components;
        bool indexed = cs is BlazorPdfIndexedColorSpace;
    ...
    private static void ApplySoftMask(BlazorPdfDict dict, IBlazorPdfXRef xref, BlazorPdfDict? resources, int width, int height, byte[] rgba)
    {
        if (dict.Get("SMask") is not BlazorPdfStream smask || smask.Dict is null)
        {
            return;
        }
        ...
        try
        {
            mdata = BlazorPdfStreamDecoder.Decode(smask);
        }
        catch
        {
            return;
        ...
        {
            var cs = BlazorPdfColorSpace.Create(dict.Get("ColorSpace", "CS"), xref, resources);
    ...
    private static void ApplyStencilMask(BlazorPdfStream mask, IBlazorPdfXRef xref, int width, int height, byte[] rgba)
    {
        ...
        try
        {
            mdata = BlazorPdfStreamDecoder.Decode(mask);
            if (filters.Contains("CCITTFaxDecode") || filters.Contains("CCF"))
            {
                mdata = BlazorPdfCcittFaxDecoder.Decode(mdata, ReadCcittParams(mask.Dict, xref, mw, mh));
            }
        }
        catch
        {
    ...
    private static double[]? ReadRawDecode(BlazorPdfDict dict)
    {
    ...
    private static List<string> GetFilterNames(BlazorPdfDict dict)
    {
        var names = new List<string>();
        object? filter = dict.Get("Filter", "F");
        if (filter is BlazorPdfName n)
        {
            names.Add(n.Value);
        }
    ...
    private static BlazorPdfCcittParams ReadCcittParams(BlazorPdfDict dict, IBlazorPdfXRef xref, int width, int height)
    {
        // /DecodeParms may be a single dictionary or an array (one per filter).
        BlazorPdfDict? parms = null;
        object? dp = dict.Get("DecodeParms", "DP");
        if (dp is BlazorPdfDict d)
        {
            parms = d;
        }
        else if (dp is List<object?> arr)
        {
            foreach (var item in arr)
            {
                if (xref.FetchIfRef(item) is BlazorPdfDict candidate && candidate.Has("K"))
                {
                    parms = candidate;
                    break;
                }
                parms ??= xref.FetchIfRef(item) as BlazorPdfDict;
            }
        }
        ...
        return new BlazorPdfCcittParams
        {
            K = GetI("K", 0),
    ...
    private static byte[]? JpegToRgba(BlazorPdfJpegImage jpeg, BlazorPdfDict dict, IBlazorPdfXRef xref)
    {
        ...
        for (int i = 0; i < w * h; i++)
        {
            ...
            (byte r, byte g, byte b) = nc switch
            {
                ...
                4 => BlazorPdfColorSpace.CmykToRgb(comps[0], comps[1], comps[2], comps[3]),
    ...
    private static double[]? ReadDecodeArray(object? value, IBlazorPdfXRef xref)
    {
    ...
    private static int GetInt(BlazorPdfDict dict, string key1, string key2)
