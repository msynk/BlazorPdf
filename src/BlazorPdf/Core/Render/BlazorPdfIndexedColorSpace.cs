...
internal sealed class BlazorPdfIndexedColorSpace : BlazorPdfColorSpace
{
    private readonly BlazorPdfColorSpace _base;
    private readonly byte[] _lookup;
    private readonly int _hival;

    private BlazorPdfIndexedColorSpace(BlazorPdfColorSpace baseCs, byte[] lookup, int hival)
    {
        _base = baseCs;
        _lookup = lookup;
        _hival = hival;
    ...
    public static BlazorPdfIndexedColorSpace Build(List<object?> arr, IBlazorPdfXRef xref, BlazorPdfDict? resources)
    {
        ...
        if (lookupObj is BlazorPdfString s)
        {
            lookup = s.Bytes;
        }
        else if (lookupObj is BlazorPdfStream stream)
        {
            lookup = BlazorPdfStreamDecoder.Decode(stream);
        }
        else
        {
            lookup = [];
        }
        return new BlazorPdfIndexedColorSpace(baseCs, lookup, hival);
    }
