...
/// </summary>
public abstract class BlazorPdfFunction
{
    ...
    public static BlazorPdfFunction? Create(object? obj, IBlazorPdfXRef xref)
    {
        ...
        {
            var parts = new List<BlazorPdfFunction>();
        ...
        BlazorPdfDict? dict = obj as BlazorPdfDict ?? (obj as BlazorPdfStream)?.Dict;
        if (dict is null)
        {
            return null;
        }
        ...
        return type switch
        {
            0 when obj is BlazorPdfStream stream => BlazorPdfSampledFunction.Build(stream, xref),
            2 => BlazorPdfExponentialFunction.Build(dict, xref),
            3 => BlazorPdfStitchingFunction.Build(dict, xref),
            4 when obj is BlazorPdfStream calc => BlazorPdfPostScriptFunction.Build(calc),
            _ => null,
        };
    }

    protected static double[] ReadNumbers(object? value, IBlazorPdfXRef? xref = null)
    {
    ...
    private sealed class ArrayFunction : BlazorPdfFunction
    {
        private readonly List<BlazorPdfFunction> _functions;
        public ArrayFunction(List<BlazorPdfFunction> functions) => _functions = functions;
