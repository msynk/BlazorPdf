...
internal sealed class PostScriptFunction : PdfFunction
{
    ...
    private PostScriptFunction(List<object> program, double[] domain, double[] range)
    {
        _program = program;
        _domain = domain;
        _range = range;
    ...
    public static PostScriptFunction? Build(PdfStream stream)
    {
        ...
        string source = System.Text.Encoding.Latin1.GetString(StreamDecoder.Decode(stream));
        ...
        return new PostScriptFunction(program, domain.Length >= 2 ? domain : [0, 1], range);
    }
