...
/// </summary>
internal sealed class BlazorPdfPostScriptFunction : BlazorPdfFunction
{
    ...
    private BlazorPdfPostScriptFunction(List<object> program, double[] domain, double[] range)
    {
        _program = program;
        _domain = domain;
        _range = range;
    ...
    public static BlazorPdfPostScriptFunction? Build(BlazorPdfStream stream)
    {
        ...
        string source = System.Text.Encoding.Latin1.GetString(BlazorPdfStreamDecoder.Decode(stream));
        ...
        return new BlazorPdfPostScriptFunction(program, domain.Length >= 2 ? domain : [0, 1], range);
    }
