...
internal sealed class BlazorPdfSampledFunction : BlazorPdfFunction
{
    ...
    private BlazorPdfSampledFunction(byte[] samples, int bps, int[] size, int inCount, int outCount,
    ...
    public static BlazorPdfSampledFunction? Build(BlazorPdfStream stream, IBlazorPdfXRef? xref = null)
    {
        ...
        byte[] samples = BlazorPdfStreamDecoder.Decode(stream);
        return new BlazorPdfSampledFunction(samples, bps, size, inCount, outCount, domain, encode, decode, range);
    }
