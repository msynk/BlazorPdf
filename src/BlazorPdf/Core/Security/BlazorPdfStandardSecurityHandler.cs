...
/// </summary>
public sealed class BlazorPdfStandardSecurityHandler
{
    ...
    private readonly BlazorPdfCipherKind _streamCipher;
    private readonly BlazorPdfCipherKind _stringCipher;

    /// <summary>The raw user-permission bits from the <c>/P</c> entry (Table 22).</summary>
    public int Permissions { get; }

    private BlazorPdfStandardSecurityHandler(byte[] fileKey, int revision, BlazorPdfCipherKind stream, BlazorPdfCipherKind str, int permissions)
    {
        _fileKey = fileKey;
        _revision = revision;
    ...
    public static BlazorPdfStandardSecurityHandler? TryCreate(BlazorPdfDict encrypt, byte[]? id0, string? password = null)
    {
        if ((encrypt.Get("Filter") as BlazorPdfName)?.Value is not "Standard")
        {
            return null;
        }
        ...
        (BlazorPdfCipherKind stream, BlazorPdfCipherKind str) = ResolveCiphers(encrypt, v);
        ...
        return new BlazorPdfStandardSecurityHandler(fileKey, r, stream, str, p);
    }
        ...
        byte[] id0, int r, int length, bool encryptMetadata, bool passwordProvided)
    {
        ...
        throw new BlazorPdfPasswordException(
    ...
    private static bool ValidateUserLegacy(byte[] key, byte[] u, byte[] id0, int r)
    {
        ...
        if (r == 2)
        {
            byte[] computed = BlazorPdfRc4.Transform(key, Padding);
            return FirstBytesEqual(computed, u, 32);
        }
        byte[] hash = Md5(Concat(Padding, id0));
        byte[] enc = BlazorPdfRc4.Transform(key, hash);
        for (int i = 1; i <= 19; i++)
        {
            enc = BlazorPdfRc4.Transform(XorKey(key, i), enc);
        }
    ...
    private static byte[]? UserPasswordFromOwner(byte[] ownerPwd, byte[] o, int r, int length)
    {
        ...
        if (r == 2)
        {
            return BlazorPdfRc4.Transform(rc4Key, userPwd);
        }
        for (int i = 19; i >= 0; i--)
        {
            userPwd = BlazorPdfRc4.Transform(XorKey(rc4Key, i), userPwd);
        }
        return userPwd;
    }
        ...
        int r, bool passwordProvided)
    {
        if (u.Length < 48)
        {
            throw new BlazorPdfPasswordException("Malformed encryption dictionary.", passwordProvided);
        }
    ...
    private byte[] Decrypt(byte[] data, int num, int gen, BlazorPdfCipherKind cipher)
    {
        switch (cipher)
        {
            case BlazorPdfCipherKind.None:
                return data;
            case BlazorPdfCipherKind.Rc4:
                return BlazorPdfRc4.Transform(ObjectKey(num, gen, aes: false), data);
            case BlazorPdfCipherKind.Aes128:
                return AesCbcDecrypt(ObjectKey(num, gen, aes: true), data);
            case BlazorPdfCipherKind.Aes256:
    ...
    private static (BlazorPdfCipherKind Stream, BlazorPdfCipherKind Str) ResolveCiphers(BlazorPdfDict encrypt, int v)
    {
        if (v >= 5)
        {
            return (BlazorPdfCipherKind.Aes256, BlazorPdfCipherKind.Aes256);
        }
        if (v == 4 && encrypt.Get("CF") is BlazorPdfDict cf)
        {
            BlazorPdfCipherKind Lookup(string filterKey)
            {
                string fname = (encrypt.Get(filterKey) as BlazorPdfName)?.Value ?? "Identity";
                if (fname == "Identity")
                {
                    return BlazorPdfCipherKind.None;
                }
                if (cf.Get(fname) is BlazorPdfDict cfDict)
                {
                    string cfm = (cfDict.Get("CFM") as BlazorPdfName)?.Value ?? "V2";
                    return cfm switch
                    {
                        "AESV2" => BlazorPdfCipherKind.Aes128,
                        "AESV3" => BlazorPdfCipherKind.Aes256,
                        "Identity" => BlazorPdfCipherKind.None,
                        _ => BlazorPdfCipherKind.Rc4,
                    };
                }
                return BlazorPdfCipherKind.Rc4;
            }
            return (Lookup("StmF"), Lookup("StrF"));
        }
        return (BlazorPdfCipherKind.Rc4, BlazorPdfCipherKind.Rc4);
    }
    ...
    private static byte[] AesCbcNoPadding(byte[] key, byte[] iv, byte[] data)
        => BlazorPdfManagedAes.CbcDecrypt(key, iv, data);

    private static byte[] AesCbcEncryptNoPadding(byte[] key, byte[] iv, byte[] data)
        => BlazorPdfManagedAes.CbcEncrypt(key, iv, data);

    private static byte[] Md5(byte[] data) => BlazorPdfManagedMd5.Hash(data);
    ...
    private static byte[] GetStringBytes(object? value) => value is BlazorPdfString s ? s.Bytes : [];
}
