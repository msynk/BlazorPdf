...
/// </summary>
public sealed class BlazorPdfMetadata
{
    ...
    internal static BlazorPdfMetadata Build(IBlazorPdfXRef xref, BlazorPdfDict? info, BlazorPdfDict catalog)
    {
        info ??= BlazorPdfDict.Empty;
    ...
    private static string? ReadXmp(IBlazorPdfXRef xref, BlazorPdfDict catalog)
    {
        if (xref.FetchIfRef(catalog.Get("Metadata")) is BlazorPdfStream stream)
        {
            try
            {
                byte[] bytes = BlazorPdfStreamDecoder.Decode(stream);
