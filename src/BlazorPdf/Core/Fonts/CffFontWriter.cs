...
internal static class CffFontWriter
{
    ...
    public static byte[]? FromType1(Type1Font t1, string psName)
    {
        try
        {
            ...
            foreach (var name in t1.CharStrings.Keys)
            {
                ...
                GlyphOutline? outline = t1.BuildOutline(name);
                if (outline is null)
                {
                    continue;
                }
    ...
    {
        string u = GlyphList.ToUnicode(name);
        return u.Length == 1 ? u[0] : (u.Length >= 1 ? char.ConvertToUtf32(u, 0) : -1);
    }

    // ----- Type2 charstring encoding -----

    private static byte[] BuildType2(GlyphOutline outline, double sx, double sy, double shx, double shy, out int advance)
    {
