...
/// </summary>
public sealed class BlazorPdfFont
{
    ...
    private readonly BlazorPdfToUnicodeCMap? _toUnicode;
    ...
    private BlazorPdfCMap _cidEncoding = BlazorPdfCMap.Identity;   // Type0 code -> CID mapping
    ...
    public BlazorPdfType3FontData? Type3 { get; private set; }
    ...
    private BlazorPdfFont(
        bool isType0, string baseFont, int firstChar, double[] widths,
        Dictionary<int, double> cidWidths, double defaultWidth, BlazorPdfToUnicodeCMap? toUnicode,
    ...
    /// <summary>Builds a <see cref="BlazorPdfFont"/> from a font dictionary.</summary>
    public static BlazorPdfFont Create(BlazorPdfDict fontDict, IBlazorPdfXRef xref)
    {
        string subtype = (fontDict.Get("Subtype") as BlazorPdfName)?.Value ?? "";
        string baseFont = (fontDict.Get("BaseFont") as BlazorPdfName)?.Value ?? "";
        BlazorPdfToUnicodeCMap? toUnicode = ReadToUnicode(fontDict);

        BlazorPdfFont font = subtype switch
        {
        ...
        BlazorPdfDict? descriptor = fontDict.Get("FontDescriptor") as BlazorPdfDict;
        if (descriptor is null && fontDict.Get("DescendantFonts") is List<object?> desc && desc.Count > 0
            && xref.FetchIfRef(desc[0]) is BlazorPdfDict cidFont)
        {
            descriptor = cidFont.Get("FontDescriptor") as BlazorPdfDict;
        }
    ...
    private static (byte[]?, string) FixCff(byte[] cff, BlazorPdfFont font, BlazorPdfDict fontDict)
    {
        ...
        byte[]? otf = BlazorPdfCffFontWriter.WrapBareCff(cff, parser.NumGlyphs, parser.FontMatrix, cmap, "BlazorPdfCff", advances);
        if (otf is null)
        {
            return (null, "cff");
        }
    ...
    private static byte[]? FixType0TrueType(byte[] program, BlazorPdfFont font, BlazorPdfDict fontDict, IBlazorPdfXRef xref)
    {
        if (fontDict.Get("DescendantFonts") is not List<object?> desc || desc.Count == 0
            || xref.FetchIfRef(desc[0]) is not BlazorPdfDict cidFont
            || (cidFont.Get("Subtype") as BlazorPdfName)?.Value != "CIDFontType2")
        {
            return null; // only TrueType-CID here; CIDFontType0 (CFF) is separate
        }

        int numGlyphs = BlazorPdfSfntGlyphMapper.ReadNumGlyphs(program);
        if (numGlyphs <= 0)
        {
            return null;
        }
        ...
        if (xref.FetchIfRef(cidFont.Get("CIDToGIDMap")) is BlazorPdfStream mapStream)
        {
            byte[] d = BlazorPdfStreamDecoder.Decode(mapStream);
        ...
        byte[]? sanitized = BlazorPdfTrueTypeSanitizer.Sanitize(program, cmap);
    ...
    private static (byte[]?, string) FixType0Cff(byte[] cff, BlazorPdfFont font)
    {
        var parser = BlazorPdfCffFontParser.Parse(cff);
        ...
        byte[]? otf = BlazorPdfCffFontWriter.WrapBareCff(cff, parser.NumGlyphs, parser.FontMatrix, cmap, "BlazorPdfCidCff", advances);
        if (otf is null)
        {
            return (null, "cff");
        }
    ...
    private static byte[] BuildGidPuaCmap(int count)
    {
        ...
        return BlazorPdfCmapBuilder.BuildUnicodeCmap(map);
    }

    private static byte[]? FixTrueType(byte[] program, BlazorPdfFont font, BlazorPdfDict? descriptor)
    {
        ...
        if (!font._isType0 && font._encoding is not null)
        {
            ...
            syntheticCmap = BlazorPdfSfntGlyphMapper.BuildSyntheticCmap(program, font._encoding, symbolic,
        ...
        return BlazorPdfTrueTypeSanitizer.Sanitize(program, syntheticCmap);
    }
    ...
    private static (byte[]?, string?) ExtractEmbedded(BlazorPdfDict? descriptor)
    {
        ...
        try
        {
            ...
            if (descriptor.Get("FontFile2") is BlazorPdfStream ttf)
            {
                return (BlazorPdfStreamDecoder.Decode(ttf), "truetype");
            }
            // FontFile3: OpenType passes through; Type1C/CIDFontType0C is bare CFF,
            // wrapped in Create once the encoding is known.
            if (descriptor.Get("FontFile3") is BlazorPdfStream ff3 && ff3.Dict is not null)
            {
                byte[] data = BlazorPdfStreamDecoder.Decode(ff3);
                return (ff3.Dict.Get("Subtype") as BlazorPdfName)?.Value == "OpenType"
                    ? (data, "opentype")
                    : (data, "cff");
            }
            ...
            if (descriptor.Get("FontFile") is BlazorPdfStream t1Stream)
            {
                byte[] raw = BlazorPdfStreamDecoder.Decode(t1Stream);
                if (BlazorPdfType1Font.Parse(raw) is { } t1)
                {
                    string psName = (descriptor.Get("FontName") as BlazorPdfName)?.Value ?? "BlazorPdfType1";
                    if (BlazorPdfCffFontWriter.FromType1(t1, Sanitize(psName)) is { } otf)
                    {
                        return (otf, "opentype");
                    }
                }
    ...
    private static BlazorPdfFont CreateSimple(BlazorPdfDict fontDict, string baseFont, BlazorPdfToUnicodeCMap? toUnicode)
    {
        ...
        bool symbolic = false;
        if (fontDict.Get("FontDescriptor") is BlazorPdfDict descriptor)
        {
        ...
        Func<int, double>? standardWidth = BlazorPdfStandardFonts.Resolve(baseFont);

        string[]? encoding = BuildSimpleEncoding(fontDict, baseFont, symbolic);

        return new BlazorPdfFont(
            isType0: false,
            baseFont,
            firstChar,
            widths.ToArray(),
    ...
    /// </summary>
    private static string[]? BuildSimpleEncoding(BlazorPdfDict fontDict, string baseFont, bool symbolic = false)
    {
        ...
        switch (enc)
        {
            case BlazorPdfName name:
                baseTable = BlazorPdfEncodings.ByName(name.Value) ?? baseTable;
                break;
            case BlazorPdfDict dict:
                if (dict.Get("BaseEncoding") is BlazorPdfName baseName)
                {
                    baseTable = BlazorPdfEncodings.ByName(baseName.Value) ?? baseTable;
                }
        ...
        if (differences is not null)
        {
            ...
            foreach (var item in differences)
            {
                ...
                else if (item is BlazorPdfName glyphName && code is >= 0 and < 256)
                {
    ...
    private static string[] DefaultBaseEncoding(string baseFont)
    {
        ...
        {
            return BlazorPdfEncodings.ZapfDingbats;
        }
        if (name.Contains("Symbol", StringComparison.OrdinalIgnoreCase))
        {
            return BlazorPdfEncodings.Symbol;
        }
        return BlazorPdfEncodings.WinAnsi;
    }
    ...
    private static BlazorPdfFont CreateType3(BlazorPdfDict fontDict, IBlazorPdfXRef xref, BlazorPdfToUnicodeCMap? toUnicode)
    {
        int firstChar = ToInt(fontDict.Get("FirstChar"), 0);
        var widths = new List<double>();
        if (fontDict.Get("Widths") is List<object?> widthArr)
        {
            foreach (var w in widthArr)
            {
                ...
                widths.Add(BlazorPdfPrimitives.ResolveNumber(xref, w));
            }
        }

        // Type3 glyphs live in glyph space; /FontMatrix maps them to text space.
        BlazorPdfMatrix fontMatrix = ReadFontMatrix(fontDict);
        ...
        BlazorPdfDict? charProcs = fontDict.Get("CharProcs") as BlazorPdfDict;
        BlazorPdfDict? resources = fontDict.Get("Resources") as BlazorPdfDict;
        font.Type3 = new BlazorPdfType3FontData(fontMatrix, charProcs, resources, encoding, xref);
        return font;
    }

    private static BlazorPdfMatrix ReadFontMatrix(BlazorPdfDict fontDict)
    {
        if (fontDict.Get("FontMatrix") is List<object?> m && m.Count >= 6)
        {
            return new BlazorPdfMatrix(ToDouble(m[0], 0.001), ToDouble(m[1], 0), ToDouble(m[2], 0),
                ToDouble(m[3], 0.001), ToDouble(m[4], 0), ToDouble(m[5], 0));
        }
        return new BlazorPdfMatrix(0.001, 0, 0, 0.001, 0, 0);
    }

    private static BlazorPdfFont CreateType0(BlazorPdfDict fontDict, IBlazorPdfXRef xref, string baseFont, BlazorPdfToUnicodeCMap? toUnicode)
    {
        ...
        if (fontDict.Get("DescendantFonts") is List<object?> descendants && descendants.Count > 0
            && xref.FetchIfRef(descendants[0]) is BlazorPdfDict cidFont)
        {
        ...
        object? enc = fontDict.Get("Encoding");
        if (enc is BlazorPdfStream cmapStream)
        {
            try
            {
                font._cidEncoding = BlazorPdfCMap.Parse(BlazorPdfStreamDecoder.Decode(cmapStream));
            }
            catch
            {
                font._cidEncoding = BlazorPdfCMap.Identity;
            }
        }
        return font;
    }

    private static void ReadCidWidths(List<object?> w, Dictionary<int, double> cidWidths, IBlazorPdfXRef xref)
    {
    ...
    private static BlazorPdfToUnicodeCMap? ReadToUnicode(BlazorPdfDict fontDict)
    {
        if (fontDict.Get("ToUnicode") is BlazorPdfStream stream)
        {
            try
            {
                return BlazorPdfToUnicodeCMap.Parse(BlazorPdfStreamDecoder.Decode(stream));
            }
            catch
            {
                return null;
    ...
    public IEnumerable<BlazorPdfGlyph> Decode(byte[] bytes)
    {
        if (_isType0)
        {
            ...
            for (int i = 0; i + step <= bytes.Length; i += step)
            {
                ...
                yield return new BlazorPdfGlyph((int)code, UnicodeFor((int)code), width, isSpace: false);
            }
        }
        else
        {
            foreach (byte b in bytes)
            {
                ...
                yield return new BlazorPdfGlyph(code, UnicodeFor(code), width, isSpace: code == 0x20);
            }
        }
    }
    ...
    private string UnicodeFor(int code)
    {
        ...
        {
            string viaName = BlazorPdfGlyphList.ToUnicode(_encoding[code]);
            if (!string.IsNullOrEmpty(viaName))
            {
                return viaName;
            }
        }
        return BlazorPdfWinAnsiEncoding.CodeToUnicode(code);
    }
    ...
    private static bool InferBold(string baseFont, BlazorPdfDict? descriptor)
    {
    ...
    private static bool InferItalic(string baseFont, BlazorPdfDict? descriptor)
    {
