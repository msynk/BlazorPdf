...
internal static class BlazorPdfCffFontWriter
{
    ...
    public static byte[]? FromType1(BlazorPdfType1Font t1, string psName)
    {
        try
        {
            ...
            foreach (var name in t1.CharStrings.Keys)
            {
                ...
                BlazorPdfGlyphOutline? outline = t1.BuildOutline(name);
                if (outline is null)
                {
                    continue;
                }
    ...
    {
        string u = BlazorPdfGlyphList.ToUnicode(name);
        return u.Length == 1 ? u[0] : (u.Length >= 1 ? char.ConvertToUtf32(u, 0) : -1);
    }

    // ----- Type2 charstring encoding -----

    private static byte[] BuildType2(BlazorPdfGlyphOutline outline, double sx, double sy, double shx, double shy, out int advance)
    {
        ...
        foreach (var seg in outline.Segments)
        {
            switch (seg.Op)
            {
                case BlazorPdfPathSeg.Kind.Move:
                {
                ...
                case BlazorPdfPathSeg.Kind.Line:
                {
                ...
                case BlazorPdfPathSeg.Kind.Curve:
                {
                ...
                case BlazorPdfPathSeg.Kind.Close:
