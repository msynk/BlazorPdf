...
internal sealed class GlyphOutline
{
    ...
    public List<PathSeg> Segments { get; } = new();
}
...
internal sealed class Type1Font
{
    ...
    public static Type1Font? Parse(byte[] data)
    {
        try
        {
    ...
    private void ParseClear(byte[] clear)
    {
        ...
        {
            Array.Copy(Encodings.Standard, Encoding, 256);
        }
        else
        {
    ...
    public GlyphOutline? BuildOutline(string glyphName)
    {
        ...
        if (ctx.Open)
        {
            ctx.Result.Segments.Add(new PathSeg { Op = PathSeg.Kind.Close });
        }
    ...
    {
        public readonly GlyphOutline Result = new();
    ...
    private void MoveOrFlex(InterpContext ctx, double dx, double dy)
    {
        ...
        ctx.Result.Segments.Add(new PathSeg { Op = PathSeg.Kind.Move, X1 = ctx.X, Y1 = ctx.Y });
        ctx.Open = true;
    }
    ...
    private void EmitFlex(InterpContext ctx)
    {
        ...
        ctx.Result.Segments.Add(new PathSeg
        {
            Op = PathSeg.Kind.Curve,
    ...
    private static void LineTo(InterpContext ctx, double x, double y)
    {
        ctx.X = x;
        ctx.Y = y;
        ctx.Result.Segments.Add(new PathSeg { Op = PathSeg.Kind.Line, X1 = x, Y1 = y });
    }
    ...
    private static void CurveRel(InterpContext ctx, double dx1, double dy1, double dx2, double dy2, double dx3, double dy3)
    {
        ...
        ctx.Result.Segments.Add(new PathSeg
        {
            Op = PathSeg.Kind.Curve, X1 = x1, Y1 = y1, X2 = x2, Y2 = y2, X3 = x3, Y3 = y3,
        });
    }
