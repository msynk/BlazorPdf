...
/// </summary>
public sealed class BlazorPdfXRef : IBlazorPdfXRef
{
    ...
    private BlazorPdfStandardSecurityHandler? _security;
    ...
    public BlazorPdfDict? Trailer { get; private set; }
    ...
    public BlazorPdfXRef(byte[] buffer)
    {
    ...
    public BlazorPdfDict? Root => Trailer?.Get("Root") as BlazorPdfDict;
    ...
    public void Parse()
    {
        ...
        catch (BlazorPdfUnsupportedEncryptionException)
        {
            throw;
        }
        catch (Exception ex)
        ...
        if (Trailer is null)
        {
            throw new BlazorPdfFormatException("No trailer found and recovery failed.");
        }
    ...
    private void SetupEncryption()
    {
        ...
        if (FetchIfRef(encRaw) is not BlazorPdfDict encDict)
        {
            return;
        }

        byte[]? id0 = null;
        if (Trailer.Get("ID") is List<object?> idArr && idArr.Count > 0 && idArr[0] is BlazorPdfString s)
        {
            id0 = s.Bytes;
        }

        _security = BlazorPdfStandardSecurityHandler.TryCreate(encDict, id0, Password);
        ...
        {
            throw new BlazorPdfUnsupportedEncryptionException(
                "The document is encrypted with an unsupported security handler or revision.");
        }
    }

    private BlazorPdfDict? ReadSection(int offset)
    {
        ...
        var probe = new BlazorPdfLexer(new BlazorPdfStream(_buffer, offset + _startOffset));
        object first = probe.GetObj();

        if (first is BlazorPdfCmd { Value: "xref" })
        {
            return ReadXRefTable(new BlazorPdfLexer(new BlazorPdfStream(_buffer, probe.Pos - 1)));
        }
    ...
    private BlazorPdfDict ReadXRefTable(BlazorPdfLexer lexer)
    {
        while (true)
        {
            object token = lexer.GetObj();
            if (token is BlazorPdfCmd { Value: "trailer" })
            {
                break;
            }
            if (ReferenceEquals(token, BlazorPdfPrimitives.EOF))
            {
                throw new BlazorPdfFormatException("Unexpected end of xref table.");
            }
            if (token is not double startObj)
            {
                throw new BlazorPdfFormatException("Malformed xref subsection header.");
            }

            object countObj = lexer.GetObj();
            if (countObj is not double countD)
            {
                throw new BlazorPdfFormatException("Malformed xref subsection count.");
            }
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                {
                    throw new BlazorPdfFormatException("Malformed xref entry.");
                }

                int num = subStart + i;
                bool free = typeTok is BlazorPdfCmd { Value: "f" };
        ...
        var parser = new BlazorPdfParser(lexer, this, allowStreams: false);
        return parser.GetObj() as BlazorPdfDict
            ?? throw new BlazorPdfFormatException("Trailer is not a dictionary.");
    }

    private BlazorPdfDict ReadXRefStream(int offset)
    {
        var parser = new BlazorPdfParser(new BlazorPdfLexer(new BlazorPdfStream(_buffer, offset + _startOffset)), this);
        if (parser.GetObj() is not BlazorPdfStream stream || stream.Dict is null)
        {
            throw new BlazorPdfFormatException("Expected an xref stream object.");
        }

        BlazorPdfDict dict = stream.Dict;
        byte[] data = BlazorPdfStreamDecoder.Decode(stream);

        if (dict.Get("W") is not List<object?> w || w.Count < 3)
        {
            throw new BlazorPdfFormatException("xref stream missing /W.");
        }
        int w0 = ToInt(w[0]);
        int w1 = ToInt(w[1]);
        int w2 = ToInt(w[2]);
        ...
        {
            throw new BlazorPdfFormatException("Invalid /W widths in xref stream.");
        }
        int entryLen = w0 + w1 + w2;
        if (entryLen == 0)
        {
            throw new BlazorPdfFormatException("Invalid /W in xref stream.");
        }
    ...
    private void MergeTrailer(BlazorPdfDict section)
    {
        if (Trailer is null)
        {
            Trailer = section;
    ...
    public object? FetchIfRef(object? value, bool suppressEncryption = false)
        => value is BlazorPdfRef r ? Fetch(r, suppressEncryption) : value;
    ...
    private object? FetchUncompressed(BlazorPdfRef reference, Entry entry)
    {
        ...
        var parser = new BlazorPdfParser(new BlazorPdfLexer(new BlazorPdfStream(_buffer, entry.Field2)), this);
    ...
    private object? DecryptObject(object? obj, int num, int gen)
    {
        switch (obj)
        {
            case BlazorPdfString s:
                return new BlazorPdfString(_security!.DecryptString(s.Bytes, num, gen));
            ...
            case BlazorPdfStream stream when stream.Dict is not null:
                DecryptDictStrings(stream.Dict, num, gen);
                // Cross-reference streams are never encrypted.
                if (BlazorPdfPrimitives.IsName(stream.Dict.Get("Type"), "XRef"))
                {
                    return stream;
                }
                ...
                return new BlazorPdfStream(decrypted, 0, decrypted.Length, stream.Dict);
    ...
    /// </summary>
    private bool HasIdentityCryptFilter(BlazorPdfDict dict)
    {
        object? filter = FetchIfRef(dict.Get("Filter"));
        var filterNames = new List<string>();
        if (filter is BlazorPdfName single)
        {
        ...
        else if (filter is List<object?> arr)
        {
            ...
            {
                if (FetchIfRef(f) is BlazorPdfName fn)
                {
        ...
        BlazorPdfDict? cryptParms = parms switch
        {
            BlazorPdfDict d => d,
            List<object?> pl when cryptIndex < pl.Count => FetchIfRef(pl[cryptIndex]) as BlazorPdfDict,
            _ => null,
        };

        // Absent /Name defaults to Identity per PDF 32000-1 §7.4.10.
        string cryptName = (cryptParms?.Get("Name") as BlazorPdfName)?.Value ?? "Identity";
        return cryptName == "Identity";
    }

    private void DecryptDictStrings(BlazorPdfDict dict, int num, int gen)
    {
    ...
    private List<object?> GetObjectStream(int streamNum)
    {
        ...
        if (Fetch(new BlazorPdfRef(streamNum, 0)) is not BlazorPdfStream stream || stream.Dict is null)
        {
            return result;
        }
        ...
        var headerLexer = new BlazorPdfLexer(new BlazorPdfStream(data));
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            var parser = new BlazorPdfParser(new BlazorPdfLexer(new BlazorPdfStream(data, objStart)), this);
    ...
    private bool HeaderMatches(int offset, int num, int gen)
    {
        try
        {
            var lexer = new BlazorPdfLexer(new BlazorPdfStream(_buffer, offset));
    ...
    private void IndexObjectStreams()
    {
        foreach (var (streamNum, entry) in _entries.ToList())
        {
            ...
            try
            {
                obj = FetchUncompressed(new BlazorPdfRef(streamNum, entry.Field3), entry);
            }
            catch
            {
                continue;
            }
            if (obj is not BlazorPdfStream s || s.Dict is null || !BlazorPdfPrimitives.IsName(s.Dict.Get("Type"), "ObjStm"))
            {
                continue;
            }
            ...
            try
            {
                data = BlazorPdfStreamDecoder.Decode(s);
            }
            catch
            {
                continue;
    ...
    private void RecoverTrailer()
    {
        ...
        BlazorPdfDict? found = FindTrailerDict();
        ...
        foreach (var (num, entry) in _entries)
        {
            ...
            try
            {
                obj = Fetch(new BlazorPdfRef(num, entry.Field3));
            }
            catch
            {
                continue;
            }
            if (obj is BlazorPdfDict d && BlazorPdfPrimitives.IsName(d.Get("Type"), "Catalog"))
            {
                Trailer ??= new BlazorPdfDict(this);
                Trailer.Set("Root", new BlazorPdfRef(num, entry.Field3));
    ...
    private BlazorPdfDict? FindTrailerDict()
    {
        var kw = "trailer"u8.ToArray();
        BlazorPdfDict? result = null;
        for (int i = 0; i + kw.Length <= _buffer.Length; i++)
        {
            ...
            try
            {
                var parser = new BlazorPdfParser(new BlazorPdfLexer(new BlazorPdfStream(_buffer, i + kw.Length)), this, allowStreams: false);
                if (parser.GetObj() is BlazorPdfDict d)
                {
    ...
    private int FindStartXRef()
    {
        ...
        for (int i = _buffer.Length - keyword.Length; i >= searchStart; i--)
        {
            ...
            {
                var lexer = new BlazorPdfLexer(new BlazorPdfStream(_buffer, i + keyword.Length));
