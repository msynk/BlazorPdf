...
/// </summary>
public sealed class BlazorPdfDocument
{
    private readonly BlazorPdfXRef _xref;
    private List<BlazorPdfPage>? _pages;
    private readonly Dictionary<BlazorPdfDict, int> _pageIndexByDict = new(ReferenceEqualityComparer.Instance);
    private IReadOnlyList<BlazorPdfOutlineItem>? _outline;
    private BlazorPdfMetadata? _metadata;
    private IReadOnlyList<string>? _pageLabels;

    private BlazorPdfDocument(BlazorPdfXRef xref) => _xref = xref;

    /// <summary>The cross-reference reader for this document.</summary>
    public BlazorPdfXRef XRef => _xref;

    /// <summary>The document catalog (<c>/Root</c>).</summary>
    public BlazorPdfDict Catalog { get; private set; } = BlazorPdfDict.Empty;
    ...
    /// </summary>
    public BlazorPdfPermissions Permissions => new(_xref.Permissions ?? -1, IsEncrypted && _xref.Permissions.HasValue);
    ...
    public IReadOnlyList<BlazorPdfOutlineItem> Outline
    {
        get
        {
            if (_outline is null)
            {
                ...
                _outline = new BlazorPdfOutlineBuilder(_xref, Catalog, _pageIndexByDict).Build();
            }
            return _outline;
        }
    }
    ...
    /// </summary>
    public BlazorPdfMetadata Metadata =>
        _metadata ??= BlazorPdfMetadata.Build(_xref, _xref.Trailer?.Get("Info") as BlazorPdfDict, Catalog);
    ...
    public IReadOnlyList<string> PageLabels =>
        _pageLabels ??= BlazorPdfPageLabelBuilder.Build(_xref, Catalog, PageCount);

    private IReadOnlyList<BlazorPdfStructElement>? _structure;
    ...
    /// </summary>
    public IReadOnlyList<BlazorPdfStructElement> StructureTree =>
        _structure ??= BlazorPdfStructTree.Build(_xref, Catalog);

    private IReadOnlyList<BlazorPdfFormField>? _formFields;

    /// <summary>
    /// The interactive form fields (<c>/AcroForm</c>) as a flat list of
    /// name/type/value, or empty when the document has no form.
    /// </summary>
    public IReadOnlyList<BlazorPdfFormField> FormFields =>
        _formFields ??= BlazorPdfAcroForm.Build(_xref, Catalog);
    ...
    {
        _ = Pages; // ensure the page-index map is populated
        return new BlazorPdfOutlineBuilder(_xref, Catalog, _pageIndexByDict).ResolveDestination(dest)?.PageNumber;
    }

    /// <summary>Parses <paramref name="bytes"/> into a document model.</summary>
    public static BlazorPdfDocument Load(byte[] bytes) => Load(bytes, null);

    /// <summary>
    /// Parses <paramref name="bytes"/> into a document model, using
    /// <paramref name="password"/> to decrypt an encrypted document (tried as
    /// both the user and owner password). Throws <see cref="BlazorPdfPasswordException"/>
    /// when a required password is missing or wrong.
    /// </summary>
    public static BlazorPdfDocument Load(byte[] bytes, string? password)
    {
        ArgumentNullException.ThrowIfNull(bytes);

        var xref = new BlazorPdfXRef(bytes) { Password = password };
        xref.Parse();

        var document = new BlazorPdfDocument(xref)
        {
            Version = ReadHeaderVersion(bytes),
        };

        document.Catalog = xref.Root
            ?? throw new BlazorPdfFormatException("Document catalog (/Root) not found.");
        ...
        if (document.Catalog.Get("Version") is BlazorPdfName catalogVersion
    ...
    private List<BlazorPdfPage> BuildPages()
    {
        var pages = new List<BlazorPdfPage>();
        if (Catalog.Get("Pages") is not BlazorPdfDict root)
        {
            return pages;
        }
    ...
    private readonly struct InheritedAttributes
    {
        ...
        public BlazorPdfDict? Resources { get; init; }
        public int? Rotate { get; init; }

        public InheritedAttributes With(BlazorPdfDict node, IBlazorPdfXRef xref)
        {
            return new InheritedAttributes
            {
                ...
                Resources = node.Get("Resources") as BlazorPdfDict ?? Resources,
                Rotate = node.Get("Rotate") is double r ? NormalizeRotation((int)r) : Rotate,
            };
        }
    }

    private void Traverse(BlazorPdfDict node, InheritedAttributes inherited,
        List<BlazorPdfPage> pages, HashSet<int> visited, int depth)
    {
        if (depth > 64)
        {
        ...
        if (node.Get("Kids") is List<object?> kids && !BlazorPdfPrimitives.IsName(typeObj, "Page"))
        {
            foreach (var kid in kids)
            {
                ...
                if (kid is BlazorPdfRef kr && !visited.Add(kr.Num))
                {
                    continue;
                }
                if (_xref.FetchIfRef(kid) is BlazorPdfDict child)
                {
        ...
        if (BlazorPdfPrimitives.IsName(typeObj, "Pages"))
        {
    ...
    private static double[]? ReadRectangle(object? value, IBlazorPdfXRef xref)
    {
