...
/// <see cref="BlazorPdfLexer"/>: arrays, dictionaries, indirect references
/// (<c>n g R</c>), indirect objects (<c>n g obj ... endobj</c>) and streams.
/// Uses two-token lookahead.
/// </summary>
public sealed class BlazorPdfParser
{
    ...
    public BlazorPdfParser(BlazorPdfLexer lexer, IBlazorPdfXRef? xref = null, bool allowStreams = true)
    {
        _lexer = lexer;
        _xref = xref;
    ...
    private static bool BufIsCmd(object buf, string cmd) => buf is BlazorPdfCmd c && c.Value == cmd;
    ...
    /// streams) are fully assembled; <c>n g R</c> becomes a <see cref="BlazorPdfRef"/>.
    /// </summary>
    public object GetObj()
    {
        ...
        {
            _depth--;
            throw new BlazorPdfFormatException("PDF object nesting too deep.");
        }
        try
        {
            return GetObjCore();
    ...
    private object GetObjCore()
    {
        ...
        if (BufIsCmd(buf1, "["))
        {
            ...
            while (!BufIsCmd(_buf1, "]") && !ReferenceEquals(_buf1, BlazorPdfPrimitives.EOF))
            {
                array.Add(GetObj());
            }
            if (ReferenceEquals(_buf1, BlazorPdfPrimitives.EOF))
            {
                throw new BlazorPdfFormatException("End of file inside array.");
            }
            Shift(); // consume ']'
            return array;
        }
        ...
        {
            Shift();
            var dict = new BlazorPdfDict(_xref);
            while (!BufIsCmd(_buf1, ">>") && !ReferenceEquals(_buf1, BlazorPdfPrimitives.EOF))
            {
                if (_buf1 is not BlazorPdfName key)
                {
                    Shift(); // skip a malformed key
                    continue;
                }
                Shift();
                if (ReferenceEquals(_buf1, BlazorPdfPrimitives.EOF) || BufIsCmd(_buf1, ">>"))
                {
                    break;
                }
            ...
            if (ReferenceEquals(_buf1, BlazorPdfPrimitives.EOF))
            {
                throw new BlazorPdfFormatException("End of file inside dictionary.");
            }
        ...
        if (buf1 is double n1 && IsInteger(n1) && _buf2 is double n2 && IsInteger(n2))
        {
            ...
            if (third is BlazorPdfCmd c)
            {
                if (c.Value == "R")
                {
                    _buf1 = _lexer.GetObj();
                    _buf2 = _lexer.GetObj();
                    return new BlazorPdfRef(num, gen);
                }
                if (c.Value == "obj")
                {
                    _buf1 = _lexer.GetObj();
    ...
    private BlazorPdfStream ParseStream(BlazorPdfDict dict)
    {
        var source = (BlazorPdfStream)_lexer.Stream;
        ...
        var streamObj = new BlazorPdfStream(buffer, dataStart, dataEnd - dataStart, dict);
